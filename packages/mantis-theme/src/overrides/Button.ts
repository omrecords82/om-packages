...
// types
import { ButtonVariantProps, ExtendedStyleProps } from '@om/mantis-shared/types/extended';

const BUTTON_COLORS = ['primary', 'secondary', 'error', 'info', 'success', 'warning'] as const;
const BUTTON_VARIANTS = ['contained', 'outlined', 'text', 'dashed', 'shadow'] as const;
 ...
 */
function generateColorVariants(theme: Theme, variantType: ButtonVariantProps) {
  return BUTTON_COLORS.reduce(
    (acc, color) => {
      const capitalizedColor = color.charAt(0).toUpperCase() + color.slice(1);
      const className = `&.MuiButton-${variantType}${capitalizedColor}`;

      acc[className] = getColorStyle({ variant: variantType, color: color as any, theme });
      return acc;
    },
...
export default function Button(theme: Theme) {
  ...
  const variants = BUTTON_VARIANTS.flatMap((variant) =>
    BUTTON_COLORS.map((color) => ({
      props: { variant, color },
      style: getColorStyle({ variant: variant as ButtonVariantProps, color: color as any, theme })
    }))
  );
