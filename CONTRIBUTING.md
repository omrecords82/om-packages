...
Standard editable lookup behavior must be consumed through `@om/ui/combo-box`. Public ComboBox selected values are `string | null`, public input values are `string`, and filtering is local, deterministic, and label-based. Typed text is not a committed value. Do not expose React Aria ComboBox, ListBox, ListBoxItem, Popover, collection, filter, or event types in public declarations. Remote fetching, custom values, grouped options, fuzzy search, and multi-select remain deferred.
